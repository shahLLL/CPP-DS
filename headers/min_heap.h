...
const std::string minHeapPeekErrorMessage = "PEEK CALLED ON EMPTY MIN HEAP";
const std::string minHeapPopErrorMessage = "POP CALLED ON EMPTY MIN HEAP";
...
class MinHeap {
    ...
    public:
        ...
        const T& peek() const {
            if(vec.empty())
                throw std::out_of_range(minHeapPeekErrorMessage);
            return vec.front();
        }

        T pop() {
            if(vec.empty())
                throw std::out_of_range(minHeapPopErrorMessage);
